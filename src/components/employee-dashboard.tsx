...
export default function EmployeeDashboard({ employeeId }: Props) {
  const [employee, setEmployee] = useState<OnchainEmployee | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [message, setMessage] = useState("");
  const [amount, setAmount] = useState("");

  async function loadEmployee(showLoader = true) {
    try {
      if (showLoader) setLoading(true);
      setError("");

      const data = await fetchOnchainBob();
      setEmployee(data);
    } catch {
      setError("No se pudo cargar el empleado on-chain.");
    } finally {
  ...
  async function handleAdvanceRequest() {
    try {
      ...
      setMessage(
        `Adelanto on-chain solicitado con éxito. Neto: $${result.request.netAmount.toLocaleString(
          "es-MX"
        )}. Fee: $${result.request.feeAmount.toLocaleString("es-MX")}.`
      );
    ...
    return (
      <main className="min-h-screen bg-slate-950 text-white px-6 py-10">
        <div className="mx-auto max-w-4xl">
          <p className="text-slate-300">Cargando empleado on-chain...</p>
        </div>
      </main>
    );
  }
      ...
      <main className="min-h-screen bg-slate-950 text-white px-6 py-10">
        <div className="mx-auto max-w-4xl">
          <p className="text-red-300">
            {error || "Empleado on-chain no encontrado."}
          </p>
        </div>
      </main>
    );
      ...
      <div className="mx-auto max-w-4xl">
        <header className="mb-10">
          ...
          <h1 className="text-3xl md:text-5xl font-bold">
            Solicitar adelanto on-chain
          </h1>
          <p className="mt-4 text-slate-300 max-w-2xl">
            Vista conectada a Testnet para solicitar adelantos reales.
          </p>
        </header>
        ...
        <section className="grid gap-6 md:grid-cols-3 mb-8">
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6 md:col-span-3">
            ...
            <h2 className="text-2xl font-bold mt-2">{employee.label}</h2>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
            <p className="text-slate-400 text-sm">Salario por ciclo</p>
            <h2 className="text-3xl font-bold mt-2">
              ${employee.salaryPerCycle.toLocaleString("es-MX")}
            </h2>
          </div>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
            ...
            <h2 className="text-3xl font-bold mt-2">
              ${employee.drawnThisCycle.toLocaleString("es-MX")}
            </h2>
          </div>
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
            ...
            <h2 className="text-3xl font-bold mt-2 text-emerald-400">
              ${employee.availableAdvance.toLocaleString("es-MX")}
            </h2>
          </div>
        </section>
        ...
        <section className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
          <h3 className="text-2xl font-semibold mb-4">
            Pedir adelanto real en Testnet
          </h3>
